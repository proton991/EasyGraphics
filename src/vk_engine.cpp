...
void VulkanEngine::init_pipelines() {
  ...
  if (!load_shader_module(getShadersPath("default_lit.frag.spv").c_str(),
                          &colorMeshShader)) {
    std::cout << "Error when building the colored mesh shader" << std::endl;
  }

  VkShaderModule texturedMeshShader;
  if (!load_shader_module(getShadersPath("textured_lit.frag.spv").c_str(),
                          &texturedMeshShader)) {
    std::cout << "Error when building the colored mesh shader" << std::endl;
  }

  VkShaderModule meshVertShader;
  if (!load_shader_module(getShadersPath("tri_mesh_ssbo.vert.spv").c_str(),
                          &meshVertShader)) {
...
void VulkanEngine::load_images() {
  ...
  vkutil::load_image_from_file(
      *this, getAssetsPath("lost_empire-RGBA.png").c_str(), lostEmpire.image);
